    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
      <div className="p-4 bg-blue-50">
        <h3 className="text-lg font-bold text-primary flex items-center">
          ...
          <LocalizedText>{translate('aiWeatherPrediction')}</LocalizedText>
        </h3>
        <p className="text-sm text-gray-600 mt-1">
          <LocalizedText>{translate('aiWeatherDescription')}</LocalizedText>
        </p>
      </div>
      ...
      <div className="p-4">
        <div className="flex flex-col gap-3">
          <div className="flex gap-2">
            ...
            >
              <option value="3">3 <LocalizedText>{translate('days')}</LocalizedText></option>
              <option value="5">5 <LocalizedText>{translate('days')}</LocalizedText></option>
              <option value="7">7 <LocalizedText>{translate('days')}</LocalizedText></option>
            </select>
            <button
            ...
            >
              {isLoading ? (
                ...
                <LocalizedText>{translate('getForecast')}</LocalizedText>
              )}
            </button>
          </div>
            ...
            <div className="mt-2">
              <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-7 gap-2">
                {forecast.forecast.map((day: any, index: number) => (
                  ...
                  >
                    <p className="text-xs font-medium"><LocalizedText>{formatDate(day.date)}</LocalizedText></p>
                    ...
                    <p className="text-xs text-gray-600"><LocalizedText>{day.condition}</LocalizedText></p>
                    {day.rainfall > 0 && (
              ...
              <div className="mt-4 bg-yellow-50 p-3 rounded-lg">
                <h4 className="font-medium text-sm"><LocalizedText>{translate('farmingAdvice')}</LocalizedText></h4>
                <ul className="mt-1 text-sm">
                  {forecast.advisories.map((advisory: string, index: number) => (
                    <li key={index} className="flex items-start mt-1">
                      ...
                      <span><LocalizedText>{advisory}</LocalizedText></span>
                    </li>
                  ))}
                </ul>
              </div>
